...
contract AutoGrowingLPTokenV4 is ERC20, Ownable, BaseHook {
    ...
    function addLiquidityV4(uint256 ethAmount) internal {
        ...
        IPoolManager.ModifyLiquidityParams memory params = IPoolManager.ModifyLiquidityParams({
            tickLower: MIN_TICK,
            tickUpper: MAX_TICK,
            liquidityDelta: int128(int256(uint256(tokenAmount))), // Convert to int128 for liquidity delta
            salt: bytes32(0) // Default salt value for position identification
        });
        ...
        if (poolKey.currency0 == tokenCurrency) {
            ...
            if (amount0 < 0) {
                ...
                CurrencySettler.settle(tokenCurrency, poolManager, address(this), tokensToSend, false);
            }
            ...
            if (amount1 < 0) {
                ...
                CurrencySettler.settle(wethCurrency, poolManager, address(this), ethToSend, true);
            }
        } else {
        ...
        if (delta.amount0() > 0) {
            poolKey.currency0.take(poolManager, address(this), uint256(uint128(delta.amount0())), false);
        }
        if (delta.amount1() > 0) {
            poolKey.currency1.take(poolManager, address(this), uint256(uint128(delta.amount1())), false);
        }
